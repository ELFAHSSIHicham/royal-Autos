...
<head>
  ...
  <style>
    .admin-wrap{display:flex;min-height:100vh}.sidebar{width:220px;background:#2d2d2d;flex-shrink:0;padding:24px 0}.sidebar-logo{padding:0 20px 20px;border-bottom:1px solid rgba(201,168,76,.15);margin-bottom:20px}.sidebar-logo div:first-child{font-family:'Cormorant Garamond',serif;font-size:16px;color:#fafafa;letter-spacing:.12em}.sidebar-logo div:last-child{font-size:7px;letter-spacing:.18em;text-transform:uppercase;color:#666;margin-top:2px}.nav-item{display:block;padding:10px 20px;font-size:9.5px;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,.4);text-decoration:none;transition:all .2s}.nav-item:hover,.nav-item.active{color:#c9a84c;background:rgba(201,168,76,.07)}.main{flex:1;background:#f5f5f5;padding:32px}
  </style>
</head>
<body>
    ...
    <div style="background:#fff;overflow:hidden">
      <table style="width:100%;border-collapse:collapse;font-size:10px;color:#5a5a5a">
        ...
        <tr style="border-bottom:1px solid #f5f5f5">
          ...
          <td style="padding:8px 14px">
            <?php $sc = ['disponible'=>'#27ae60','reserve'=>'#f39c12','vendu'=>'#95a5a6','maintenance'=>'#2980b9']; ?>
            <span style="background:<?= $sc[$v['statut']] ?? '#aaa' ?>;color:#fff;padding:2px 8px;font-size:7.5px;letter-spacing:.06em;text-transform:uppercase"><?= htmlspecialchars($v['statut']) ?></span>
          </td>
          <td style="padding:8px 14px">
